...
/*远程连接访问资料*/
function fopen_url($url,$arr=false)
{
	...
	if($arr==true){$file_content=json_decode($file_content,TRUE);}
	return $file_content;
}
 ...
 *	获取远程内容https
 */
function curl_https($url, $data=array(), $header=array(),$echo=true, $timeout=30){
$ch = curl_init();
...
return json_decode($response,$echo);
