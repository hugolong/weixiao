...
switch ($ac) {
	...
	case "addprofile" :
		...
		$token = "true";//$ro->test_login($account['username'], $account['password']);
		if ($token != '') {
			...
			updatetable(tname('open_member_weixin'), array (
				'state' => 0,
				'password' => ''
			), array (
				'username' => $account['username'],
	...
	case "editprofile" :
		...
		if ($token != '') {
			...
			updatetable(tname('open_member_weixin'), $setarr, array (
				'op_uid' => $_SGLOBAL['uid'],
				'id' => $id
			));
			updatetable(tname('weixin_member'), array (
				'state' => 1
			), array (
				'op_wxid' => $id
			));
	...
	case "pusheditprofile" :
		...
		if ($token != '') {
			$beenadd = getcount(tname('open_member_weixin'), array (
				'username' => $username,
				'state' => 1
			));
			...
			$setarr = array (
				...
				'weixin_name' => $info['nickname'],
				'username' => getstr($_POST['username']
			), 'password' => getstr($_POST['password']), 'signature' => $info['signature'], 'country' => $info['country'], 'province' => $info['province'], 'city' => $info['city'], 'verifyInfo' => $info['verifyInfo'], 'bindUserName' => $info['bindUserName'], 'account' => $info['account'], 'fakeid' => $info['fakeid'], 'state' => 1,);
			if ($_SGLOBAL['db']->getone('select id from ' . tname('open_member_pushweixin') . ' where op_uid=' . $_SGLOBAL['uid'])) {
				updatetable(tname('open_member_pushweixin'), $setarr, array (
					'op_uid' => $_SGLOBAL['uid']
				));
			} else {
			...
			updatetable(tname('open_member_user'), array (
				'weixin_state' => 0
			), array (
				'op_uid' => $_SGLOBAL['uid']
			));
	...
	case "manage" :
		...
		foreach ($modules as $k => $v) {
			$modules[$k]['enabled'] = getcount(tname('weixin_module'), array (
				'op_uid' => $op_uid,
				'op_wxid' => $id,
				'mid' => $v['mid'],
				'enabled' => 1
	...
	case "del" :
		...
		break;
	default :
		$total = getcount(tname('open_member_weixin'), array (
			'op_uid' => $_SGLOBAL['uid']
		));
...
function check_role($id) {
	global $_SGLOBAL;
	if (!getcount(tname('open_member_weixin'), array (
			'id' => $id,
			'op_uid' => $_SGLOBAL['uid']
		))) {
		exit ();
