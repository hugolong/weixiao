...
switch ($ac) {
	...
	case "addprofile" :
		...
		) ? intval($_POST['sort_order']) : 0, 'btn_type' => intval($_POST['btn_type']) ? intval($_POST['btn_type']) : 0, 'btn_name' => getstr($_POST['btn_name']), 'keyword' => getstr($_POST['keyword']), 'url' => getstr($_POST['url']), 'addtime' => $_SGLOBAL['timestamp'],);
		$count = getcount(tname('open_member_weixin_custommenu'), array (
			'wxid' => $wxid,
			'parent_id' => 0
		));
	...
	case "editprofile" :
		...
		$setarr = array (
			'sort_order' => intval($_POST['sort_order']
		) ? intval($_POST['sort_order']) : 0, 'btn_type' => intval($_POST['btn_type']) ? intval($_POST['btn_type']) : 0, 'btn_name' => getstr($_POST['btn_name']), 'keyword' => getstr($_POST['keyword']), 'url' => getstr($_POST['url']),);
		updatetable(tname('open_member_weixin_custommenu'), $setarr, array (
			'id' => $id
		));
		gourl('wx_custommenu.php?id=' . $wxid);
		exit;
	...
	default :
		...
		check_role($id);
		$smarty->assign('id', $id);
		$total = getcount(tname('open_member_weixin_custommenu'), array (
			'wxid' => $id
		));
		...
		foreach ($list as $k => $v) {
			...
			if ($v['btn_type'] == 2)
				$list[$k]['btn_type_show'] = '网址跳转';
			$list[$k]['son'] = $_SGLOBAL['db']->getall('select * from ' . tname('open_member_weixin_custommenu') . ' where parent_id=' . $v['id'] . ' order by sort_order');
			foreach ($list[$k]['son'] as $key => $value) {
				if ($value['btn_type'] == 1)
					$list[$k]['son'][$key]['btn_type_show'] = '关键词';
				if ($value['btn_type'] == 2)
					$list[$k]['son'][$key]['btn_type_show'] = '网址跳转';
			}
		}
